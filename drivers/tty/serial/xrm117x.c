			   ...
			   int irq, unsigned long flags)
{
	...
	irq_set_irq_type(gpio_to_irq(irq), IRQF_TRIGGER_FALLING);
	irq = gpio_to_irq(irq);
...
static int xrm117x_spi_probe(struct spi_device *spi)
{
	...
	if (spi->dev.of_node) {
		const struct of_device_id *of_id =
				of_match_device(xrm117x_dt_ids, &spi->dev);
...
static const struct spi_device_id xrm117x_spi_id_table[] = {
